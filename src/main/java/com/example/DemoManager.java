...
import java.util.*; import java.io.*;  // multiple imports on same line (bad)

/**
 * Terrible example class full of conventions violations.
 */
public class DemoManager { public int x=5; public static String gLOBAL_name="X";
...
System.out.println("b small");
    ...
    public void addThing(Object o){
        ...
        for(int j=0;j<3;j++){
            ...
            int zz=0;
        while(zz<7){
        if(zz==3){System.out.println("three");}
            else if(zz==4){System.out.println("four");}
                else { System.out.println("other");}
        zz++;
    }

    try { File f = new File("something.txt"); if(f.exists())
...
Collections.sort(l); // raw list, no generics
    ...
    public void overlyComplexMethod() {
        ...
        Random R=new Random();
        int v=R.nextInt(100);
        if(v>50){System.out.println("big");}
